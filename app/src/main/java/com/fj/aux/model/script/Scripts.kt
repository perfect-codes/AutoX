package com.fj.aux.model.script
...
import com.fj.aux.external.shortcut.Shortcut
import com.fj.aux.external.shortcut.ShortcutActivity
import com.fj.aux.ui.edit.EditActivity
...
object Scripts {
    ...
    fun createShortcut(scriptFile: ScriptFile) {
        Shortcut(GlobalAppContext.get()).name(scriptFile.simplifiedName)
                ...
                .extras(Intent().putExtra(com.fj.aux.external.ScriptIntents.EXTRA_KEY_PATH, scriptFile.path))
                .send()
    }
    ...
    fun run(file: ScriptFile): ScriptExecution? {
        return try {
            com.fj.aux.autojs.AutoJs.getInstance().scriptEngineService.execute(file.toSource(),
    ...
    fun run(source: ScriptSource): ScriptExecution? {
        return try {
            com.fj.aux.autojs.AutoJs.getInstance().scriptEngineService.execute(source, ExecutionConfig(workingDirectory = Pref.getScriptDirPath()))
        } catch (e: Exception) {
            e.printStackTrace()
            Toast.makeText(GlobalAppContext.get(), e.message, Toast.LENGTH_LONG).show()
            null
    ...
    fun runWithBroadcastSender(file: File): ScriptExecution {
        return com.fj.aux.autojs.AutoJs.getInstance().scriptEngineService.execute(ScriptFile(file).toSource(), BROADCAST_SENDER_SCRIPT_EXECUTION_LISTENER,
                ExecutionConfig(workingDirectory = file.parent))
    }
    ...
    fun runRepeatedly(scriptFile: ScriptFile, loopTimes: Int, delay: Long, interval: Long): ScriptExecution {
        ...
        return com.fj.aux.autojs.AutoJs.getInstance().scriptEngineService.execute(source, ExecutionConfig(workingDirectory = directoryPath,
